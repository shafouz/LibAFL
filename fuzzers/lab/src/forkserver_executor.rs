...
use libafl::{executors::Executor, prelude::Observer};
use libafl::{executors::ExitKind, prelude::Tokens};
use libafl::{executors::HasObservers, prelude::Input};
use libafl::{observers::UsesObservers, prelude::HasTargetBytes};
use libafl::{prelude::MapObserver, state::State};
use libafl::{prelude::ObserversTuple, state::HasExecutions};
use nix::libc;
use std::{
    env,
    ffi::{OsStr, OsString},
...
use libafl_bolts::{
    ...
    AsSlice, AsSliceMut, Error, Truncate,
};
use nix::{
    sys::{
...
fn fs_opt_get_mapsize(x: i32) -> i32 {
    let ret = ((x & 0x00fffffe) >> 1) + 1;
    eprintln!("DEBUGPRINT[2]: forkserver_executor.rs:55: ret={:#?}", ret);
    ret
}
...
// tries to kill child then server
...
impl Forkserver {
    ...
    pub fn with_kill_signal(
        ...
        // sets forkserver env
        // set memory limit / disable core dumps
        // sets session id ?? makes the forkserver a daemon
        // calls dup2 on stdin
        // same for other pipes
        // spawn
...
pub struct CustomForkserverExecutor<OT, S, SP>
where
    SP: ShMemProvider,
{
    target: OsString,
...
impl<OT, S, SP> Debug for CustomForkserverExecutor<OT, S, SP>
where
    OT: Debug,
    SP: ShMemProvider,
{
...
impl CustomForkserverExecutor<(), (), UnixShMemProvider> {
    /// Builder for `ForkserverExecutor`
    #[must_use]
    pub fn builder() -> CustomForkserverExecutorBuilder<'static, UnixShMemProvider> {
        CustomForkserverExecutorBuilder::new()
    }
}

impl<OT, S, SP> CustomForkserverExecutor<OT, S, SP>
where
    OT: ObserversTuple<S>,
    S: UsesInput,
    SP: ShMemProvider,
...
impl<OT, S, SP> UsesState for CustomForkserverExecutor<OT, S, SP>
where
    S: State,
    SP: ShMemProvider,
{
...
impl<OT, S, SP> UsesObservers for CustomForkserverExecutor<OT, S, SP>
where
    OT: ObserversTuple<S>,
    S: State,
    SP: ShMemProvider,
...
impl<OT, S, SP> HasObservers for CustomForkserverExecutor<OT, S, SP>
where
    OT: ObserversTuple<S>,
    S: State,
    SP: ShMemProvider,
...
impl<EM, OT, S, SP, Z> Executor<EM, Z> for CustomForkserverExecutor<OT, S, SP>
where
    OT: ObserversTuple<S>,
    SP: ShMemProvider,
...
pub struct CustomForkserverExecutorBuilder<'a, SP> {
...
impl<'a, SP> CustomForkserverExecutorBuilder<'a, SP> {
    ...
    pub fn build<OT, S>(
        &mut self,
        observers: OT,
    ) -> Result<CustomForkserverExecutor<OT, S, SP>, Error>
    where
        OT: ObserversTuple<S>,
        S: UsesInput,
    ...
    pub fn build_dynamic_map<A, MO, OT, S>(
        ...
        mut map_observer: A,
        other_observers: OT,
    ) -> Result<CustomForkserverExecutor<(A, OT), S, SP>, Error>
    where
...
impl<'a> CustomForkserverExecutorBuilder<'a, UnixShMemProvider> {
    ...
    /// If `debug_child` is set, the child will print to `stdout`/`stderr`.
    #[must_use]
    pub fn new() -> CustomForkserverExecutorBuilder<'a, UnixShMemProvider> {
        CustomForkserverExecutorBuilder {
            program: None,
            arguments: vec![],
            envs: vec![],
            debug_child: false,
    ...
    ) -> CustomForkserverExecutorBuilder<'a, SP> {
        CustomForkserverExecutorBuilder {
...
impl<'a> Default for CustomForkserverExecutorBuilder<'a, UnixShMemProvider> {
    fn default() -> Self {
        Self::new()
    }
}
